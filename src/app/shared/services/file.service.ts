...
export class FileService {
  ...
  generateReport = (title: string, headers: any[], data: any, fileName: string, information: any, footerTitle?: string, showLogo: boolean = false) => {
    let doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.width || doc.internal.pageSize.getWidth();
    if (showLogo) {
      // doc.circle(20, 20, 10, 'S')
      doc.addImage(information.profilePic ? information.profilePic : '../../assets/user-default.png', 14, 14, 25, 25);
      // doc.addImage(this.logo, pageWidth / 2.3, 4, 25, 25);
    }

    const designation = this.designations.find(x => x.key === information.designation).viewValue;

    doc.setFontSize(10)
    doc.text(`${information.firstName} ${information.lastName}`, 45, 25);
    doc.text(`${designation}`, 45, 31);
